...
Wyvern Drive is a personal cloud drive backed by Discord — files split into chunks, AES-256-GCM encrypted at rest, served from one per-user Discord webhook — and it wears the Framer dark-canvas world the way Framer wears it: a near-black canvas, oversized white display type with hard negative tracking, and one chromatic accent (blue) that is reserved for links, focus, and selection — never a fill. The app is a poster at the edges (login, share links) and a quiet instrument in the middle (the drive itself): scanability, density, and the familiar cloud-drive idioms outrank expression once the user is inside.
...
- 5px spacing base (5/10/15/20/30/40), radius scale from 4px chips to 100px pills, 15–20px cards, 30px spotlight.
...
**The Gradient-Is-Cards Rule.** Gradients belong to the spotlight card alone. A gradient applied to a whole section, a button, or a background is a drift; gradients are cards.
...
- **Chrome:** Fixed 240px side rail on canvas with a hairline right border; wordmark top, Drive/Settings nav, user chip + logout at the bottom. Mobile collapses to a 56px canvas app bar with a temporary drawer. The shell renders a keyboard skip link targeting `#main-content` and a visible route header built from the page title.
- **Content column:** max 520px for centered public surfaces (login, share); the drive page runs full-width inside the rail with a 20px gutter; the selection bar and empty-state spotlight center at 560px.
...
- **Entry views:** desktop table (canvas rows, surface-1 hover, surface-2 selected, hairline-soft dividers) or grid (surface-1 cards, 15px radius, hover lifts to surface-2 + light-edge); mobile stacks full-width cards. EntryActions reveal on row hover on desktop and stay visible on touch.
...
- **Login & Share — poster voice.** The two public surfaces speak first: BrandLockup centered, the 62px display-hero tagline on login, a single white pill CTA on the dark canvas. Persuade-lite: one statement, one action.
- **Drive — Operate-first.** Inside, the world goes quiet: dense tables and grids, the toolbar task cluster, quota readout, floating upload queue. Familiar cloud-drive idioms outrank expression; the one gradient moment — the violet spotlight empty state — is the memorable moment that pays off the poster outside.
...
- **Upload queue:** floating bottom-right surface-2 panel, 20px radius, light-edge; jobs show a 6px white progress bar on a white-10% track; done = success-green check glyph. Safe-area aware (`bottom: max(16px, env(safe-area-inset-bottom))`) and clamped inside the viewport edge below 412px (`maxWidth: calc(100vw - 16px)`, `left: 8` fallback).
...
### Navigation
- **Side rail:** canvas, hairline right border. Wordmark = BrandLockup: folder glyph in a surface-2 circle + Mona Sans 500 "Wyvern Drive" (compact variant for the mobile app bar). Nav links ink-muted → selected surface-2 pill (lift, not color); hover surface-1. `aria-current="page"` on the active item.
...
### Feedback & Status
- **ScreenLoader:** full-surface canvas progressbar labeled "Loading" (or a bespoke label like "Checking storage") — boot and per-page load state, no spinner choreography.
- **ErrorNotice:** `role="alert"` outlined surface-1 alert with ink text and a Retry action — errors are announced, never silent.
- **EntryActions:** row-level action cluster (rename, move, copy, share, delete) revealed on hover on desktop rows/tiles, always visible on touch cards.
